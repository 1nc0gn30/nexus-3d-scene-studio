...
class NexusStudioRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        # 5. Static File or Embedded UI Fallback
    ...
    def do_POST(self) -> None:
        ...
        # 2. API: Export Shape (OBJ, STL, Three.js, HTML)
        if path == "/api/export":
            try:
...
4. `model.json`: Three.js BufferGeometry format for WebGL integration.
5. `viewer.html`: Self-contained interactive 3D WebGL viewer with OrbitControls.
6. `audit.json`: Polygon budget, bounding box, and diagnostic metrics.
"""
